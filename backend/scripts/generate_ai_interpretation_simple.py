...
# -*- coding: utf-8 -*-
"""
AI 诗词解读批量生成脚本 - 简化版（使用 httpx 直接调用 API）
"""
...
async def generate_interpretation(
    ...
    if not ANTHROPIC_API_KEY:
        print(f"  ⚠ 跳过 {title}：缺少 ANTHROPIC_API_KEY")
        return None
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"  ✗ {title} API 错误：{response.status_code} - {response.text[:100]}")
            return None
...
async def main():
    ...
    print("=" * 50)
    print("AI 诗词解读生成器（样例版）")
    print("=" * 50)

    if not ANTHROPIC_API_KEY:
        print("\n⚠ 警告：缺少 ANTHROPIC_API_KEY")
        print("请在 .env 文件中添加：ANTHROPIC_API_KEY=sk-ant-xxxxx")
        print("\n临时设置可运行：export ANTHROPIC_API_KEY='sk-ant-xxxxx'")
        return
